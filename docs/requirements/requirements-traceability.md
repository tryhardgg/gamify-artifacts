...
| 7 | История изменений KR | Нет логирования | ⚠️ Расходится | Создать таблицу `kr_history` или использовать soft-delete + лог | `db-schema.md` |
...
| 11 | История KR: все изменения | Нет логирования | ⚠️ Расходится | Таблица `kr_history`: `event_type` (create, update_value, update_weight, delete), `old_value`, `new_value`, `timestamp` | `db-schema.md` |
| 12 | Objective не считает прогресс без распределённых весов | Прогресс считается всегда | ⚠️ Расходится | Состояние «черновик»: KR созданы, веса не распределены. Прогресс = 0. | `domain-model.md`, `db-schema.md` |
